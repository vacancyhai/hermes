...
from app.services.notifications import DAILY_EMAIL_LIMIT, NotificationService
...
def test_send_push_deduplicates_by_fingerprint():
    ...
    with patch.object(svc, "_send_fcm", return_value=True) as mock_fcm:
        svc._send_push(
            "notif-1", "user-1", "Title", "Message", datetime.now(timezone.utc)
        )
...
def test_send_fcm_success():
    ...
    with patch("firebase_admin.credentials.Certificate") as mock_cert, patch(
        "firebase_admin.initialize_app"
    ) as mock_init, patch("firebase_admin.messaging.Message") as mock_msg, patch(
...
def test_send_email_instant_mode():
    ...
    with patch.object(svc, "_load_preferences", return_value={}), patch.object(
        svc, "_create_in_app"
    ), patch.object(svc, "_send_push"), patch.object(
        svc, "_send_email"
...
def test_schedule_delayed_email():
    ...
    with patch("app.tasks.notifications.deliver_delayed_email") as mock_task:
        svc._schedule_delayed_email(
            "notif-1", "user-1", "Subject", "template.html", {"key": "val"}
        )
...
def test_send_instant_mode_all_channels():
    ...
    with patch.object(svc, "_create_in_app") as mock_in_app, patch.object(
        svc, "_send_push"
    ) as mock_push, patch.object(svc, "_send_email") as mock_email, patch.object(
        svc, "_send_whatsapp"
    ) as mock_wa:
...
def test_send_staggered_mode_delays_email_whatsapp():
    ...
    with patch.object(svc, "_create_in_app") as mock_in_app, patch.object(
        ...
        svc, "_schedule_delayed_email"
    ) as mock_sched_email, patch.object(
        svc, "_schedule_delayed_whatsapp"
    ) as mock_sched_wa:
...
def test_send_respects_email_preference():
    ...
    with patch.object(
        svc, "_load_preferences", return_value={"email": False}
    ), patch.object(svc, "_create_in_app") as mock_in_app, patch.object(
        svc, "_send_push"
...
def test_send_respects_push_preference():
    ...
    with patch.object(
        svc, "_load_preferences", return_value={"push": False}
    ), patch.object(svc, "_create_in_app") as mock_in_app, patch.object(
        svc, "_send_push"
    ) as mock_push:
...
def test_send_returns_notification_id():
    ...
    with patch.object(svc, "_create_in_app"), patch.object(
        svc, "_send_push"
