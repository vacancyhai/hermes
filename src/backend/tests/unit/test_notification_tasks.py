...
def test_send_new_job_notifications_with_followers():
    """Inserts notifications and queues emails for followers."""
    ...
    # Mock user preferences check for _queue_email_for_user
    user_pref_result = MagicMock()
    user_pref_result.fetchone.return_value = ("user@test.com", "Test User", {})

    side_effects = [job_result, profiles_result, MagicMock(), user_pref_result]
...
def test_notify_priority_subscribers_with_trackers():
    """Inserts notifications for priority trackers."""
    user_id = str(uuid.uuid4())
    session = MagicMock()
    job_result = MagicMock()
    job_result.fetchone.return_value = ("job-id", "SSC CGL", "ssc-cgl", "SSC")
    ...
    session.execute.side_effect = [job_result, apps_result, MagicMock()]
    ctx = MagicMock()
    ctx.__enter__ = MagicMock(return_value=session)
    ctx.__exit__ = MagicMock(return_value=False)
