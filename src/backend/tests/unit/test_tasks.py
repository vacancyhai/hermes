...
def test_extract_job_from_pdf_no_text():
    """PDF with no extractable text returns error dict."""
    with patch("app.services.pdf_extractor.extract_text_from_pdf", return_value="   "), \
         patch("app.tasks.jobs.Session"):
        from app.tasks.jobs import extract_job_from_pdf
        # Call the underlying function directly (bypass Celery)
        task = extract_job_from_pdf
        result = task.run("/fake/empty.pdf", "admin-uuid")

    assert result["status"] == "error"
    assert "no extractable text" in result["detail"].lower()


def test_extract_job_from_pdf_success_with_ai():
    """Successful PDF → AI extraction → DB insert path."""
    extracted = {
        "job_title": "UPSC IAS 2024",
        "organization": "UPSC",
    ...
    with patch("app.services.pdf_extractor.extract_text_from_pdf", return_value="Some real PDF text"), \
         patch("app.services.ai_extractor.extract_job_data", return_value=extracted), \
         patch("app.tasks.jobs.Session", return_value=ctx):
        from app.tasks.jobs import extract_job_from_pdf
        result = extract_job_from_pdf.run("/fake/upsc.pdf", "admin-uuid-123")
...
def test_extract_job_from_pdf_ai_fallback():
    """When AI extraction returns None, fallback draft is created."""
    session = MagicMock()
    ctx = MagicMock()
    ctx.__enter__ = MagicMock(return_value=session)
    ctx.__exit__ = MagicMock(return_value=False)

    with patch("app.services.pdf_extractor.extract_text_from_pdf", return_value="PDF text content"), \
         patch("app.services.ai_extractor.extract_job_data", return_value=None), \
         patch("app.tasks.jobs.Session", return_value=ctx):
        from app.tasks.jobs import extract_job_from_pdf
        result = extract_job_from_pdf.run("/fake/job.pdf", "admin-id")
