...
async def test_verify_email_otp_invalid(client: AsyncClient):
    ...
    resp = await client.post(
        "/api/v1/auth/verify-email-otp", json={"email": email, "otp": "999999"}
    )
...
async def test_complete_registration_with_valid_password(
    client: AsyncClient, test_redis
):
    ...
    await test_redis.set(
        data_key, json.dumps({"full_name": "Valid User", "phone": None})
    )

    with patch("app.routers.auth.jwt.decode") as mock_decode, patch(
        "app.firebase.init_firebase", return_value=True
    ), patch("firebase_admin.auth") as mock_fb:

        mock_decode.return_value = {"purpose": "email_verified", "email": email}
...
async def test_set_password_for_google_user(client: AsyncClient, db):
    ...
    with patch("app.firebase.init_firebase", return_value=True), patch(
        "firebase_admin.auth"
    ) as mock_fb:
...
async def test_change_password_with_weak_new_password_fails(
...
async def test_update_phone_marks_as_unverified(
    client: AsyncClient, user_token: str, db, test_user
):
...
async def test_verify_phone_otp_with_invalid_otp(client: AsyncClient, user_token: str):
    ...
    resp = await client.post(
        "/api/v1/users/me/verify-phone-otp",
        headers=auth_header(user_token),
...
async def test_link_email_to_phone_only_account(client: AsyncClient, db):
    ...
    with patch("app.firebase.init_firebase", return_value=True), patch(
        ...
        resp = await client.post(
            "/api/v1/users/me/link-email-password",
            headers=auth_header(token),
            json={"email": new_email, "password": "LinkPass123!"},
        )
...
async def test_link_email_to_user_with_existing_email_fails(
    client: AsyncClient, user_token: str
):
...
async def test_link_email_with_existing_email_fails(client: AsyncClient, db, test_user):
    ...
    resp = await client.post(
        "/api/v1/users/me/link-email-password",
        headers=auth_header(token),
        json={"email": existing_email, "password": "TryLink123!"},  # Already exists
    )
    assert resp.status_code == 400
    json_resp = resp.json()
...
async def test_check_phone_availability_available(client: AsyncClient):
    ...
    resp = await client.post(
        "/api/v1/auth/check-phone-availability", json={"phone": "+919999999999"}
    )
    assert resp.status_code == 200
...
async def test_check_phone_availability_registered(client: AsyncClient, db):
    ...
    resp = await client.post(
        "/api/v1/auth/check-phone-availability", json={"phone": phone}
    )
    assert resp.status_code == 200
